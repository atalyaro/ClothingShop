    ...
    return this.http.get("http://localhost:1000/auth/checkinguserstatus", {
      ...
      (res: any) => {
        if (this._data.loggedUser.access) {
          ...
          this._data.sentencestatus = `hello ${this._data.loggedUser.private_name}, you have open cart from ${res.dateofcart} how's cost ${res.meanwhileprice}$`
          this._data.cartprice = res.meanwhileprice
        } else if (!res.err && res.last_order_date) {
          this._data.buttonstatus = "Start Shopping"
          this._data.sentencestatus = `hello ${this._data.loggedUser.private_name}, your last purchase was on ${res.last_order_date.slice(0, 10)}`
        } else if (!res.err) {
          this._data.buttonstatus = "Start Shopping"
          this._data.sentencestatus = `hello ${this._data.loggedUser.private_name}, welcome to your first purchase`
        }
      }, err => {
        this._data.sentencestatus = ""
        console.log(err)
